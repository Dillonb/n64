...
void r4300i_step(r4300i_t* cpu) {
    ...
    switch (decode(cpu, pc, instruction)) {
        ...
        exec_instr(LUI,   mips32_lui)
        exec_instr(ADDI,  mips32_addi)
        exec_instr(ADDIU, mips32_addiu)
        exec_instr(ANDI,  mips32_andi)
        exec_instr(LBU,   mips32_lbu)
        exec_instr(LW,    mips32_lw)
        exec_instr(BEQ,   mips32_beq)
        exec_instr(BLEZL, mips32_blezl)
        exec_instr(BNE,   mips32_bne)
        exec_instr(BNEL,  mips32_bnel)
        exec_instr(CACHE, mips32_cache)
        exec_instr(SB,    mips32_sb)
        exec_instr(SW,    mips32_sw)
        exec_instr(ORI,   mips32_ori)
        exec_instr(JAL,   mips32_jal)
        exec_instr(SLTI,  mips32_slti)
        exec_instr(BEQL,  mips32_beql)
        exec_instr(XORI,  mips32_xori)

        // Coprocessor
        exec_instr(CP_MTC0, mips32_mtc0)

        // Special
        exec_instr(SPC_SRL,   mips32_spc_srl)
        exec_instr(SPC_SLLV,  mips32_spc_sllv)
        exec_instr(SPC_SRLV,  mips32_spc_srlv)
        exec_instr(SPC_JR,    mips32_spc_jr)
        exec_instr(SPC_MFHI,  mips32_spc_mfhi)
        exec_instr(SPC_MFLO,  mips32_spc_mflo)
        exec_instr(SPC_MULTU, mips32_spc_multu)
        exec_instr(SPC_ADD,   mips32_spc_add)
        exec_instr(SPC_ADDU,  mips32_spc_addu)
        exec_instr(SPC_AND,   mips32_spc_and)
        exec_instr(SPC_SUBU,  mips32_spc_subu)
        exec_instr(SPC_OR,    mips32_spc_or)
        exec_instr(SPC_XOR,   mips32_spc_xor)
        exec_instr(SPC_SLT,   mips32_spc_slt)
        exec_instr(SPC_SLTU,  mips32_spc_sltu)

        // REGIMM
        exec_instr(RI_BGEZL,  mips32_ri_bgezl)
        exec_instr(RI_BGEZAL, mips32_ri_bgezal)
