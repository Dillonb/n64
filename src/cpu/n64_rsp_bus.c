...
word n64_rsp_read_word(rsp_t* rsp, word address) {
    return word_from_byte_array_unaligned((byte*) &rsp->sp_dmem, (address & 0xFFF));
}

void n64_rsp_write_word(rsp_t* rsp, word address, word value) {
    word_to_byte_array_unaligned((byte*) &rsp->sp_dmem, address & 0xFFF, value);
}

half n64_rsp_read_half(rsp_t* rsp, word address) {
    return half_from_byte_array_unaligned((byte*) &rsp->sp_dmem, address & 0xFFF);
}

void n64_rsp_write_half(rsp_t* rsp, word address, half value) {
    half_to_byte_array_unaligned((byte*) &rsp->sp_dmem, address & 0xFFF, value);
}
