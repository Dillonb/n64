...
MIPS_INSTR(mips_tlbwi) {
    ...
    printf(
            ...
            "page mask: 0x%08X\n",
            index,
            cpu->cp0.entry_hi.raw,
            cpu->cp0.entry_lo0.raw,
            cpu->cp0.entry_lo1.raw,
            cpu->cp0.page_mask.raw
            );
    if (index >= 32) {
        logfatal("TLBWI to TLB index %d", index)
    }
    cpu->cp0.tlb[index].entry_hi.raw  = cpu->cp0.entry_hi.raw & (~cpu->cp0.page_mask.raw);
    cpu->cp0.tlb[index].entry_lo0.raw = cpu->cp0.entry_lo0.raw;
    cpu->cp0.tlb[index].entry_lo1.raw = cpu->cp0.entry_lo1.raw;
    cpu->cp0.tlb[index].page_mask.raw = cpu->cp0.page_mask.raw;
}
