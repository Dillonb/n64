...
void fill_temp_code_rsp(u16 address, rsp_code_overlay_t* overlay) {
    int instructions_left_in_block = -1;

    temp_code_len = 0;
#ifdef N64_LOG_COMPILATIONS
    printf("Starting a new RSP block:\n");
#endif

    for (int i = 0; i < MAX_RSP_BLOCK_LENGTH || instructions_left_in_block > 0; i++) {
        u16 instr_address = (address + (i << 2)) & 0xFFF;
        //u16 next_instr_address = (instr_address + 4) & 0xFFF;
...
//#endif
    ...
    // If we hit the max length, make sure the last instruction is not a branch
    if (temp_code_len == MAX_RSP_BLOCK_LENGTH && is_branch(temp_code[MAX_RSP_BLOCK_LENGTH - 1].category)) {
        logwarn("Filled temp_code buffer, but the last instruction was a branch. Stripping it out.");
        temp_code_len--;
    }
}
...
void v2_compile_new_block(
    ...
    ir_context_reset();
    ir_context.target = COMPILER_TARGET_CPU;
...
void v2_compile_new_block_rsp(rsp_dynarec_block_t* block, rsp_code_overlay_t* overlay, u16 address) {
    fill_temp_code_rsp(address, overlay);

    /* TODO: does the RSP ever idle loop?
    if (detect_idle_loop(virtual_address)) {
        block->run = idle_loop_replacement;
        block->guest_size = 0;
        block->host_size = 0;
    ...
    ir_context_reset();
    ir_context.target = COMPILER_TARGET_RSP;
    ir_context.block_start_virtual = address;
    ir_context.block_start_physical = address;
    ...
    for (int i = 0; i < temp_code_len; i++) {
        u64 instr_address = address + (i << 2);
        emit_rsp_instruction_ir(temp_code[i].instr, i, instr_address);
    }

    if (!ir_context.block_end_pc_ir_emitted && temp_code_len > 0) {
        ir_instruction_t* end_pc = ir_emit_set_constant_u16(address + (temp_code_len << 2), NO_GUEST_REG);
        ir_emit_set_block_exit_pc(end_pc);
    }
