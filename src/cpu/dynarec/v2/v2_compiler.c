...
void compile_ir_set_constant(dasm_State** Dst, ir_instruction_t* instr) {
    host_emit_mov_reg_imm(Dst, instr->allocated_host_register, instr->set_constant);
}
...
void compile_ir_set_cp0(dasm_State** Dst, ir_instruction_t* instr) {
    if (is_constant(instr->set_cp0.value)) {
        ...
        host_emit_mov_cp0_reg(Dst, instr->set_cp0.reg, instr->set_cp0.value->allocated_host_register);
    }
}
